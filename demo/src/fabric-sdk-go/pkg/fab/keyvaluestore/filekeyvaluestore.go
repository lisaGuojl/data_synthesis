...
//support paas read pem from dir rather than with filename
func (fkvs *FileKeyValueStore) dirFileName(path string) (string, error) {
	if path == "" {
		return "", errors.New("File path is empty")
	}
	files, err := ioutil.ReadDir(path)
	if err != nil {
		return "", errors.New("read directory failed")
	}
	for _, f := range files {
		if f.IsDir() {
			continue
		}
		fullName := filepath.Join(path, string(filepath.Separator), f.Name())
		return fullName, nil
	}
